    ...
    <div class="page-leftheader">
        ...
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item" aria-current="page">
                <a href="/getucon/accounting/{{$type}}">getucon GmbH Accounting</a></li>
        ...
        <div class="col-md-12 col-lg-12">
            <div class="card">
                <div class="card-header d-flex justify-content-between">
                    ...
                    <div class="text-right">
                        <div class="float-right ml-4">
                            <a href="{{url('/getucon/accounting/' . $type)}}" class="btn btn-info"><i class="fa fa-backward mr-1"></i>{{ucfirst(trans('words.back'))}}</a>
                        </div>
                        @if($page_type == "update")
                        <div class="float-right ml-4">
                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-12 col-lg-12">
                            <form class="form-horizontal" action="{{$page_type=="update"?url("/getucon/accounting/update/".$type."/".$accounting->id):url("/getucon/accounting/add/".$type)}}" method="POST" id="main-form">
                                @csrf

                                <div class="row">
                                    @if(!isset($ref_no))
                                        @include("getucon-accounting.standart-left-section")
                                        @include("getucon-accounting.standart-right-section")
                                    @else
                                        @if(!$isCopy)
                                            <input type="hidden" name="reference_no" value="{{$accounting->no}}">
                                            <input type="hidden" name="reference_type" value="{{$accounting->type}}">
                                        @endif
                                        @include("getucon-accounting.custom-left-section")
                                        @include("getucon-accounting.custom-right-section")
                                    @endif
                                </div>
                ...
                <div class="row w-100 m-0">
                    <div class="col-lg-12 col-md-12 my-3 d-flex justify-content-end">
                        ...
                        <a href="/getucon/accounting/{{$type}}" class="btn btn-danger">Cancel</a>
                ...
                @if($page_type == "update")
                    @if($mail_status == 1)
                        @include("getucon-accounting.mail")
                    @elseif($type == "invoice" && $mail_status == 2)
                        @include('getucon-accounting.invoice-reminder-section')
                    @endif
                @endif
            </div>
        </div>
                        ...
                        @if($mail_logs->count() > 0)
                            <table class="table w-100" id="mail-logs-table">
                                ...
                                @foreach($mail_logs as $mail_log)
                                    @php
                                        $user = \App\User::query()->find($mail_log->send_by);
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <div class="col-md-12">
                        Invoice will be canceled! This action has not a rollback!<br><span class="text-danger">Please realize what you are doing!</span>
                    </div>
                </div>
                <div class="row mt-2">
                    <div class="col-md-12">
...
const trumbowConf = {
    ...
    function calculateTotalAmount() { // KDV dahil hesaplama burada yapılıyor.
        let total_amount = 0;
        ...
        let with_kdv = total_amount + (total_amount * (0.19));
        $("#with-kdv").val(numberFormat(with_kdv, 2, ",", "."));
    }
    ...
    function changeTicketStatus() {
        confirmModal("Are you sure that you want to change ticket's status to \"Invoiced\"?", "Change Ticket Status", "Change", "Close").then(function() {
            toggleLoader(true);
            $.ajax({
                url: "/getucon/accounting/change-ticket-status",
                type: "POST",
                data: {
                    _token: "{{ csrf_token() }}",
                    id: {{ $accounting->id }}
        ...
        if(a === 0) {
            ...
            let html = `<div class="col-md-1 pl-0">
                        ...
                        <div class="col-md-1">
                            <div class="form-group row">
                                <div class="col-md-12">
                                    <input class="form-control privateValidateControl accounting-item-discount" data-type="currency" type="text" name="items[${position_number}][discount]">
                                </div>
                            </div>
                        </div>
...
`
    ...
    function updateTicket() {
        ...
        $.ajax({
            url:'/getucon/accounting/update-ticket',
            type: 'POST',
            data: {
                ticket_id: ticket_id,
    ...
    @if($page_type == "update" && $type == "invoice" && $mail_status == 2)
    function deleteReminderAttachment(attachment_id) {
        confirmModal('Reminder attachment will be deleted!',"Are you sure?","Delete Attachment","Close","#0275d8","#d9534f").then(function() {
            $.ajax({
                url:'/getucon/accounting/delete-attachment',
                type: 'POST',
                data: {
    ...
    function formSubmit() {
        ...
        if(!reminder) {
            confirmModal("Achtung, es wird eine Rechnungs-E-Mail an <br> TO:" + email +(ccSplitEmails[0]!="" ? "<br> CC:" : "") +ccSplitEmails.map((item)=>item+" ")+(bccSplitEmails[0]!="" ? "<br> BCC:" : "")+bccSplitEmails.map((item)=>item+" ") +"<br> versendet!","Are you sure?","Ok","Cancel").then(function() {
                toggleLoader(true);
    ...
    $('#reminder-attachment-dropzone').dropzone({
        ...
        init: function() {
            ...
            if(mockFile){
                let real_dp = this;
                $.each(mockFile,function (index,set){

                    set.status=Dropzone.SUCCESS;
                    set.accepted = true;
                    real_dp.emit("addedfile",set);
                    real_dp.emit("complete",set);
                    real_dp.files.push(set);
                    $("#reminder-attachment-response").append("<input type='hidden' name='reminder-attachment-dropzone["+set.size+"]' value='"+set.name+"'>");
                });
            }
    ...
    $(document).ready(function() {
        ...
        $("#show-reminder-section").on('change', function() {
            if($(this).is(':checked')) {
                ...
                $.ajax({
                    url:"/getucon/accounting/retrieve-invoice-details",
                    type:"POST",
                    async:false,
                    data: {
      ...
      @if($page_type === "update")
        $(document).on("click", ".delete-ticket", function() {
            ...
            confirmModal("Are you sure that you want to delete this position?", "Are you sure?", "Delete", "Close", "#0275d8", "#d9534f").then(function() {
                ...
                $.ajax({
                    url: "/getucon/accounting/delete-ticket",
                    type: "post",
                    data:{
        ...
        $(document).on("click", ".delete-main-ticket", function() {
            confirmModal("Are you sure that you want to delete this position?", "Are you sure?", "Delete", "Close", "#0275d8", "#d9534f").then(function() {
                ...
                $.ajax({
                    url: "/getucon/accounting/delete-main-ticket",
                    type: "post",
                    data:{
                        "_token":$('meta[name="csrf-token"]').attr('content'),
                        "accounting_id":{{ $accounting->id }}
 ...
 @endif
        ...
        $(document).on("keyup change", ".accounting-item-quantity", function() {
            ...
            if(unit_price !== "" && discount !== "" && quantity !== "") {
        ...
        $(document).on("keyup change", ".accounting-item-unit-price,.accounting-item-discount", function() {
        ...
        $("#deadline-day").on("change", function () { // Burada gün seçimine göre "Deadline" hesaplanıyor.
            let day = $(this).val();
            let date = $('#date').val();
        ...
        function disableAll() {
            isAllDisabled = 1;
            $("#main-form").find(":input").not("textarea[name='internal_info'],#hidden-submit-btn,input[name='_token'],input[name='isDisabledAll'],input[name='repeat_date'],select[name='repeat_reminder'],#save-and-close,select[name='status']").prop("disabled", true);
            $(".add-row-btn").remove();
            $(".delete-btn").remove();
        ...
        @if($type=="invoice") // "Type: Invoice" Start
        @if($accounting->reminder_setted != null || $accounting->storno != null || $accounting->offer_no || $accounting->proforma_no)
        disableAll();
        @endif

        $(document).on("click", "#cancel-invoice", function() {
            let invoice_no = $(this).data("invoice-no");
            $("#cancel-invoice-modal").modal("show");
            $("#cancel-invoice-accept-btn").data("invoice-no", invoice_no);
        });
        ...
        $(document).on("click", "#cancel-invoice-accept-btn", function() {
            ...
            if(reason === "" || to === null) {
                $("#cancel-reason").addClass("error-border");
                $("input[name='to_for_storno']").addClass("error-border");
                return false;
            }
            toggleLoader(true);
            $.ajax({
                url: "/getucon/accounting/cancel-invoice",
                type: "POST",
                data: {
        ...
        $('#result').val(numberFormat(parseFloat($('#with-kdv').val().replace(/\./g, "").replace(/\,/g, ".")) - parseFloat($('#total_payment').val()), 2, ',', '.') + ' €');
        var total_payment = parseFloat($('#total_payment').val());
        var invoice_amount = parseFloat($('#with-kdv').val().replace(/\./g, "").replace(/\,/g, "."));
        ...
        @endif // "Type: Invoice" End


        @if($type=="offer")
        @if($accounting->invoice_no || $accounting->proforma_no || $accounting->is_storno)
        disableAll();
        @endif

        @elseif($type=="proforma")
        @if($accounting->invoice_no || $accounting->is_storno || $mail_status == 1 || $accounting->offer_no)
        disableAll();
        @endif
        @endif
        ...
        $("#show-send-mail").on("change", function() {
            if($(this).is(":checked")) {
                ...
                $.ajax({
                    url: "/getucon/accounting/get-requested-accounting",
                    type: "POST",
                    async: false,
                    data: {
                    ...
                    success: function (response) {
                        if(response.success === 1) {
                            ...
                            if(accounting.type === "invoice") {
                                $("#mail-subject").val("Storno-Rechnung: ST-" + accounting.storno_no + " zu RG-" + accounting.no + " ");
                                organization.email = organization.email.replaceAll(";", ",");
                                accounting.storno.email_cc = accounting.storno.email_cc.replaceAll(";", ",");
                                accounting.storno.email_bcc = accounting.storno.email_bcc.replaceAll(";", ",");

                                if(organization.accounting_to) {
                                    $("#email_to").val(organization.accounting_to);
                                }
                                else {
                                    $("#email_to").val(organization.email);//reminder robotta organizasyon mailine göndermek zorunlu olduğu için
                                }

                                $("#email_cc").val(accounting.storno.email_cc);
                                $("#email_bcc").val(accounting.storno.email_bcc);
                            }
                            else {
                                if(accounting.type === "offer") {
                                    $("#mail-subject").val("Angebot: " + $("#accounting-no").val() + " ");
                                }

                                if(accounting.type === "proforma") {
                                    $("#mail-subject").val("Proforma: " + $("#accounting-no").val() + " ");
                                }

                                if(organization.accounting_to) {
                                    $("#email_to").val(organization.accounting_to); // Organizasyon'un mailini al.
                                }
                                else {
                                ...
                                if(organization.accounting_cc) {
                                    $("#email_cc").val(organization.accounting_cc);
                                }

                                if(organization.accounting_bcc) {
                                    $("#email_bcc").val(organization.accounting_bcc);
                                }
                            }

                            // Oluşturulan dosyaları sunucudan alarak Dropzone'a koy.
                            let mockFiles = [];
        ...
        $("#send-button").on("click", function() {
            ...
            confirmModal('Are you sure? Mail will be sent!',"","Ok","Cancel").then(function() {
                toggleLoader(true);

                $.ajax({
                    url: "/getucon/accounting/send-mail",
                    type: "POST",
                    dataType: "json",
                    data: {
                        _token: "{{csrf_token()}}",
