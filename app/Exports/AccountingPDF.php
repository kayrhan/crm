...
use tFPDF;
class AccountingPDF extends tFPDF
{
    //a4 sizes 210 x 297
    public $pageH = 297;
    public $pageW = 210;
    public $headerH = 50;
    public $footerH = 30;
    public $pageBreakmm = 36;//footer + 6
    ...
    function Header()
    {
            ...
            if($this->firstHeader == 1) {
                 ...
                 $this->Cell(($this->pageW/2)-($this->page_indent),5," Übertrag","L,B",1,"L");
     ...
     function Footer()
    {
        ...
        $matrix = [
            ...
            ["Amtsgericht Frankfurt","HRB 126687","USt-IdNr.: DE351617962" ,"GF.: Cem Güler",""],
        ...
        for( $m =0;$m<count($matrix);$m++){
            for($n = 0;$n<count($matrix[$m]);$n++) {
                ...
                $this->Cell($newRowW, $row_height, $matrix[$m][$n], 0, 1, "L");
    ...
    public function getInvoicesPositions(){
        ...
        for($i=0;$i<count($starts);$i++){
             ...
             *  ardından end değerine göre çizim yaptırıyoruz*/
                if ($starts[$i]["page"] != $ends[$i]["page"]) {
                    ...
                    //ara toplam
                    $this->PageBreakTrigger = $this->pageH;//page break triggerını disable ediyoruz çünkü Zwischensumme'nun yeri belli
                    $this->SetXY($this->page_indent, -$this->pageBreakmm);
                    $this->SetDrawColor(0, 0, 0);
                    $this->SetFont("Arial", "", 9);
                    $this->Cell(($this->pageW - ($this->page_indent * 2)) / 2, 5, "Zwischensumme", "L,B,T");
        ...
        //şimdilik dursun
        /*
        dd(["starts"=>$starts,"ends"=>$ends,"total_page"=>$this->total_page,"excluded_pages"=>$excluded_pages]);
        for($i=1;$i<$this->total_page;$i++){
            //dd(["total_page"=>count($starts),"sayfa"=>count($ends)]);
            if($starts[$i-1]["current"]!=$i) {
                //if(!isset($starts[$i])) {
                //echo $i."\n";
                $this->page = $i;
                ...
                //y1 = $this->headerH+8+8+5
                //y2 = $this->thresh
                //$this->drawInvoiceTable($this->headerH+8+8+5,$this->thresh);


                //ara toplam start
                $this->PageBreakTrigger = 290;
                $this->SetXY($this->page_indent, 247);
                $this->SetDrawColor(0, 0, 0);
                $this->SetFont("Arial", "", 9.5);
                $this->Cell(($this->pageW - ($this->page_indent * 2)) / 2, 5, "Zwischensumme", "L,B,T");
                $this->SetXY($this->page_indent + ($this->pageW - ($this->page_indent * 2)) / 2, -$this->footerH - 20);
                $this->Cell(($this->pageW - ($this->page_indent * 2)) / 2, 5, self::convert_price($this->sub_totals[$i]), "R,B,T", 1, "R");
                $this->PageBreakTrigger = 247;
                //ara toplam end
            }
        }
        */
        ...
        $this->SetX($this->page_indent);
        $this->Cell(self::w[0]+self::w[1]+self::w[2]+self::w[3]+self::w[4],5,"Gesamt Netto","L,B,T");
        ...
        $this->SetX($this->page_indent);
        $this->Cell(self::w[0]+self::w[1]+self::w[2]+self::w[3],5,"zzgl. 19,00 % USt. auf ","L,B");
        ...
        $this->Cell(self::w[5]+$borderPosition,5,$this->accounting->tax_rate.' EUR',"R,B",1,"R"); // kdv tutarı
        ...
        $this->Cell( self::w[0]+self::w[1]+self::w[2]+self::w[3]+self::w[4]-$borderPosition,4,"Gesamtbetrag","L,R,B");
    ...
    public function create_pdf($data){
        ...
        if($this->accounting->type == "offer"){
                $this->accounting->raw_no = "Angebot Nr. ".$this->accounting->no;
                $this->logo = $this->company->logo;
                $this->info_mail = $this->company->info_mail;

        }elseif ($this->accounting->type == "proforma"){
            $this->accounting->raw_no = "Proformarechnung Nr. ".$this->accounting->no;
            $this->logo = $this->company->logo;
            $this->info_mail = $this->company->info_mail;
        }elseif ($this->accounting->type == "invoice"){
            if($this->accounting->storno_no){
                $this->accounting->raw_no = "Storno Rechnung Nr. ".$this->accounting->storno_no;
            }
            else{
                $this->accounting->raw_no = "Rechnung Nr. ".$this->accounting->no;
            }
            $this->logo = "getucon.png";
            $this->info_mail = "info@getucon.de";
        }
        ...
        if($this->accounting->storno_no){
            ...
            $this->MultiCell($this->pageW - (2 * $this->page_indent),4,"zu: Rechnung Nr. ".$this->accounting->no,0,"L");
    ...
    public function getInvoiceInfo(){
        ...
        if($this->accounting->storno_no==null) {
            $this->SetXY($infoX, $tmp_Y);
            $this->Cell($cell_width / 2, 4, "Lieferdatum:", 0, 0, "L");
    ...
    public function getInvoiceTableHeader(){
        ...
        $header = ["Pos.","Menge","Beschreibung",["Einzelpreis","EUR"],["Rabatt","%"],["Gesamtpreis","EUR"]];
        $constY = $this->GetY();
    ...
    public function getInvoiceFooter(){
        ...
        if(($this->accounting->type=="invoice" && $this->accounting->storno_no==null) || $this->accounting->type=="proforma"){

            $this->MultiCell($this->pageW-($this->page_indent*2),$this->textHeight,"",0,"L");
            $this->SetX($this->page_indent);
            $this->MultiCell($this->pageW-($this->page_indent*2),$this->textHeight,"Zahlbar innerhalb ".$this->accounting->deadline_day." Tagen ohne Abzug "."(".$this->accounting->deadline.")",0,"L");
