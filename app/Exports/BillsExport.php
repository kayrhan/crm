...
class BillsExport implements FromCollection, WithHeadings, ShouldAutoSize, WithEvents {
    ...
    public function registerEvents(): array {
        $total_row = count($this->ticket_ids); // Satır oluşturmak için kaç Ticket olduğunu sayıyoruz.
        return [
            AfterSheet::class  => function (AfterSheet $event) use ($total_row) {
                ...
                for($i = 2; $i <= $total_row + 2; $i++) { // Satırlarını rengini tek veya çift olup olmamasına göre burada çizdiriyoruz.
                    if($i % 2 == 0) {
                ...
                for($i = 1; $i <= $total_row + $waste; $i++) { // Kenarları burada çizdiriyoruz.
                ...
                $columns = ["I", "J", "K", "L", "M", "N"]; // Efor zamanlarının bulunduğu sütunları alıyoruz.
                foreach($columns as $column) {
                    $event->sheet->getComment($column . "1")->getText()->createTextRun('Discount is included.');
                }
                $event->sheet->getStyle('H' . ($total_row + $waste))->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('FFe6f58a');
                $event->sheet->getStyle('I' . ($total_row + $waste) . ":N" . ($total_row + $waste))->getFill()->setFillType(Fill::FILL_SOLID)->getStartColor()->setARGB('FF54f276');
            },
        ];
    }
    ...
    public function collection() {
        ...
        $row[] = array(); // Footer öncesi bir satır atlıyoruz.
        $row[] = $footer;
